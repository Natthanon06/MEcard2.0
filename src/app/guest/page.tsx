...
export default function GuestPage() {
    ...
    fetch("/api/guest/get-scans")
      ...
      .then((result) => {
        if (result.success && result.data) {
          // 💡 ดักไว้ว่าถ้า API เผลอส่งมาหลายใบ ให้ดึงมาแค่ "ใบแรกใบเดียว" เสมอ
          const latestData = Array.isArray(result.data) ? result.data[0] : result.data;
          
          // ดึง cardData ออกมาแสดงผล
          setScannedData(latestData?.cardData || latestData);
        }
        setIsLoading(false);
      })
      .catch((err) => {
  ...
  return (
    <div className="min-h-screen bg-black flex flex-col font-sans">
      ...
      {/* --- ส่วนล่าง: ข้อมูลเพื่อนที่สแกนใบเดียว (Contextual Data) --- */}
